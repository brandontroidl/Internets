...
def _self_test_argon2() -> None:
    """Round-trip + benchmark the configured argon2id params.

    Auto-degrades if hashing exceeds ~1 s (operator-facing latency hit);
    warns (but does not change) if hashing is suspiciously fast (<50 ms).
    The bump-down only affects the in-memory params for THIS process —
    stored hashes always carry their own params, so a degraded run still
    verifies older, stronger hashes correctly.
    """
    global _ARGON2_DEFAULT_MEM_MIB, _ARGON2_DEFAULT_TIME
    try:
        from argon2 import PasswordHasher
    except ImportError:
        log.info("argon2 self-test skipped — argon2-cffi not installed")
        return

    sample = "self-test-sentinel-password"
    for attempt in range(4):
        ...
        # Round-trip check
        if not ph.verify(h, sample):
            raise RuntimeError("argon2 self-test failed: verify returned False")
        if elapsed > _SLOW_HASH_THRESHOLD_S:
            # Too slow — degrade and retry.  Drop memory first (linear vs
            # both memory bandwidth and runtime), then time_cost.
            new_mem = max(_ARGON2_MEM_MIN_MIB, int(mem_kib / 1024 * 0.75))
            new_t   = t_cost
            if new_mem == mem_kib // 1024 and t_cost > _ARGON2_TIME_MIN:
                new_t = t_cost - 1
            log.warning(
                "argon2 hash took %.2fs (>%.2fs); degrading params "
                "%d MiB/t=%d/p=%d → %d MiB/t=%d/p=%d",
                elapsed, _SLOW_HASH_THRESHOLD_S,
                mem_kib // 1024, t_cost, p,
                new_mem, new_t, p,
            )
            _ARGON2_DEFAULT_MEM_MIB = new_mem
            _ARGON2_DEFAULT_TIME    = new_t
            # Honour env override if it was forcing us high — clear it
            # for this process only so the next call uses the new defaults.
            os.environ.pop("INTERNETS_ARGON2_MEM_MIB", None)
            os.environ.pop("INTERNETS_ARGON2_TIME", None)
            if new_mem <= _ARGON2_MEM_MIN_MIB and new_t <= _ARGON2_TIME_MIN:
                log.error("argon2 already at floor; cannot degrade further")
                return
            continue
        if elapsed < _FAST_HASH_THRESHOLD_S:
            log.warning(
                "argon2 hash took only %.3fs (<%.3fs) with %d MiB/t=%d/p=%d — "
                "parameters may be too weak for 2026 GPU-class attackers; "
                "consider raising INTERNETS_ARGON2_MEM_MIB or INTERNETS_ARGON2_TIME",
                elapsed, _FAST_HASH_THRESHOLD_S, mem_kib // 1024, t_cost, p,
            )
        log.info("argon2 self-test OK: %d MiB / t=%d / p=%d in %.3fs",
                 mem_kib // 1024, t_cost, p, elapsed)
        return
    log.error("argon2 self-test exhausted degradation attempts")


def _self_test() -> None:
    """Run round-trip + benchmark for the chosen-default algorithm chain.

    Called automatically by ``main()`` after hashing.  Idempotent — safe
    to call from a smoke test.
    """
    _self_test_argon2()
