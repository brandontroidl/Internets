...
def aqi_fmt(aqi: Optional[int], category: str = "") -> str:
    """Format an AQI value with category label."""
    if aqi is None:
        return "AQI N/A"
    cat = f" ({category})" if category else ""
    return f"AQI {aqi}{cat}"


# ── Marine ───────────────────────────────────────────────────────────

def wave_fmt(height_m: Optional[float], period_s: Optional[float] = None,
             direction: str = "") -> str:
    """Format wave height in meters and feet with optional period and direction."""
    if height_m is None:
        return "Waves N/A"
    parts = [f"Waves {height_m:.1f}m / {height_m * 3.281:.1f}ft"]
    if period_s is not None:
        parts.append(f"{period_s:.0f}s")
    if direction:
        parts.append(f"from {direction}")
    return " ".join(parts)


def swell_fmt(height_m: Optional[float], period_s: Optional[float] = None,
              direction: str = "") -> str:
    """Format swell height in meters and feet with optional period and direction."""
    if height_m is None:
        return "Swell N/A"
    parts = [f"Swell {height_m:.1f}m / {height_m * 3.281:.1f}ft"]
    if period_s is not None:
        parts.append(f"{period_s:.0f}s")
    if direction:
        parts.append(f"from {direction}")
