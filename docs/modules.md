...
(`internets.py:465`). A file with no `setup` is rejected at load
(`internets.py:466`).
...
7. **Conflict check**: any command word already owned by a *different* module
   ...
   rejected (`internets.py:468`). (A reload of the same module is fine because
...
`reload_module` is `unload` then `load` (`internets.py:500`). Because step 5
